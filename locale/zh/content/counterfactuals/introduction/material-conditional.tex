...
在英语中最简单的形式中，条件句是形如「if \dots then \dots」的语句，其中\dots{}本身是语句，例如「如果管家做了这件事，那么园丁是无辜的。」在入门逻辑课程中，我们学会用 $\lif$ 联结词来符号化条件句：符号化由 \dots 指示的部分，例如用!!{formula} $!A$ 和~$!B$，而整个条件句则符号化为 $!A \lif !B$。

联结词 $\lif$ 是 \emph{真值函项的}（truth-functional），即 $!A \lif !B$ 的真值——$\True$ 或 $\False$——由 $!A$ 和~$!B$ 的真值决定：当 $!A$ 为假或 $!B$ 为真时，$!A \lif !B$ 为真，否则为假。相对于真值指派~$\pAssign v$，我们定义 $\pSat{v}{!A \lif !B}$ 当且仅当 $\pSat/{v}{!A}$ 或 $\pSat{v}{!B}$。具有这种语义的联结词 $\lif$ 被称为 \emph{实质条件句}（material conditional）。
...
\begin{align}
  ...
  \intertext{实质条件句由其后件，以及由对其前件的否定所衍推：}
  !B & \Entails !A \lif !B\\
  \lnot !A & \Entails !A \lif !B
  \intertext{一个为假的实质条件句等价于其前件与其后件的否定的合取：如果 $!A \lif !B$ 为假，则 $!A \land \lnot !B$ 为真，反之亦然：}
  ...
  \intertext{我们总能加强前件，即该条件句是 \emph{单调的}（monotonic）：}
