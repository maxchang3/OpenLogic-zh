...
\zhFullName{David K. Lewis}{大卫·刘易斯}{刘易斯}引入了\emph{严格条件句}~$\strictif$，并论证说，对应蕴含的是它——而非实质条件句。在真性模态逻辑中，$!A \strictif !B$ 可定义为 $\Box(!A \lif
...
\begin{prob}
  通过给出如下各项的 \Log{S5}-证明，表明这些有效的衍推关系对严格条件句成立：
  \begin{enumerate}
...
\end{align}
如果你把 $\strictif$ 想作「蕴含」，这些便不是问题。逻辑衍推关系终归是数学事实，因此不能是偶然的。但如果你想用 $\strictif$ 作为一个旨在刻画「如果……那么……」的逻辑联结词，它们确实会带来麻烦，尤其是最后两条。因为当然存在着偶然真或偶然假的「如果……那么……」语句——事实上，它们通常既非必然也非不可能。
