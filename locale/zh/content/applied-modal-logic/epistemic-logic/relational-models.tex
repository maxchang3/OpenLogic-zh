...
\begin{defn}
  ...
  \item $V$ 是一个函数，它把每组!!{propositional variable}~$p$ 指派给一个由可能世界构成的集合 $V(p)$。
  \end{enumerate}
  当 $R_a ww'$ 成立时，我们称 $w'$ 对~$w$ 而言是 $a$ \emph{可及的}。当 $w \in V(p)$ 时，我们称 $p$ 在~$w$ 处\emph{为真}。
\end{defn}

运作机制与正规模态逻辑的运作机制完全相同，只是加入了更多的可及关系。对某个给定的主体，我们通常将其可及关系解释为刻画了其信息状态的某些方面。例如，我们常常把 $R_a ww'$ 当作表达 $w'$ 与~$a$ 在~$w$ 处的信息是\emph{一致的}。或者换一种说法，在~$w$ 处，$a$ 无法区分世界 $w$ 与世界~$w'$。
