...
\begin{defn}
  ...
  \end{enumerate}
  为使事情更简单，我们还通常假定：当一个历史在~$C$ 中，其所有后缀也在~$C$ 中。例如，如果 $s_1$、$s_2$、~$s_3$ 是~$C$ 中的一条序列，那么 $s_2$、$s_3$ 与~$s_3$ 也是。此外，当两个状态 $s_i$ 与~$s_j$ 出现在一条序列~$\sigma$ 中时，称 $s_i \prec_\sigma s_j$ 成立，当且仅当 $i < j$。当 $t \in V(p)$ 时，称 $p$~在~$t$ 处\emph{为真}。
\end{defn}
