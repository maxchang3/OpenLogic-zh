...
\olfileid[zh]{nml}{prf}{mpr}

\olsection{\Log{K} 中的更多证明}

现在让我们用 \olref[der]{sec} 中引入的简化方法，再看一些~\Log{K} 中关于!!{derivability}的例子。
...
5. &  $\Log{K} \Proves \Box(!A \lif !B) \lif (\Diamond!A \lif \Diamond!B)$ & 以 $\Diamond$ 代 $\lnot\Box\lnot$.
\end{derivation}
\end{proof}
...
\begin{proof}
  ...
  \Diamond !B)$ & 以 $\Diamond$ 代 $\lnot\Box\lnot$.
  \end{derivation}
\end{proof}
...
\begin{proof}
  \begin{derivation}
    ...
    以 $\Diamond$ 代 $\lnot\Box\lnot$\\
    5. & $\Log{K} \Proves \Diamond!B \lif \Diamond(!A \lor !B)$ & 同理\\
...
\begin{proof}
  \begin{derivation}
    ...
    \Diamond!B \lif \Diamond!A)$ & 以 $\Diamond$ 代 $\lnot\Box\lnot$\\
    7. & $\Log{K} \Proves \Diamond(!A\lor!B) \lif (\Diamond!B \lor \Diamond!A)$ & \PL, 6. \\
  \end{derivation}
\end{proof}

\begin{prob}
  证明下列!!{derivability}断言成立：
  \begin{enumerate}
