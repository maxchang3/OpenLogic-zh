...
\end{equation}
\Ax{W} 定义传递且逆良基的框架类。一个关系是良基的，如果不存在无穷序列
$w_1$, $w_2$, \dots{} 使得 $Rw_2w_1$, $Rw_3w_2$, \dots。例如，$\Nat$~上的关系 $<$ 是良基的，而 $\Int$~上的关系 $<$ 则不是。一个关系是逆良基的，当且仅当其逆关系是良基的。因此，逆良基关系就是那些不存在无穷序列 $w_1$, $w_2$, \dots{}
使得 $Rw_1w_2$, $Rw_2w_3$, \dots 的关系。

然而，并不存在一阶!!{formula}来定义传递的
逆良基关系。因为设 $\Sat{M}{!F}$ 当且仅当 $R =
\Assign{Q}{M}$ 是传递且逆良基的。令 $!A_n$ 为!!{formula}
\[
...
\dots, k\}$ 上的 $<$ 是传递且逆良基的，
故 $\Sat{M_k}{!F}$。而由构造知，对所有 $i \le k$ 有 $\Sat{M_k}{!A_i}$。
由一阶逻辑的紧致性定理，$\Gamma$ 在某个!!{structure}~$\Struct{M}$ 中是可满足的。由假设，由于
$\Sat{M}{!F}$，关系 $\Assign{Q}{M}$ 是逆
良基的。但显然，$\Assign{a_1}{M}$, $\Assign{a_2}{M}$,
\dots{} 会构成一个被逆良基性所排除的那种无穷序列。
