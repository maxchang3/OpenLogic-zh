...
!!^{tableau}是某些特定（向下分支的）!!{signed formula}树，即由真值符（$\True$ 或
...
$\sFmla{\False}{!A}$ 时，我们称这个支是 \emph{闭的}。若!!a{tableau}中的每一个
...
我们省去外层的 $\tuple{\ }$，并用~$.$'s 而非 $,$'s 分隔各个!!{element}。若 $\sigma$ 是一个前缀，则
...
直观上讲，前缀为!!a{tableau}某支上的!!{formula}指定了模型中一个可能满足这些公式的世界，而若 $\sigma$ 指定某个
