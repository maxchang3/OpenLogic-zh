...
\olsection{布劳威尔–海廷–科莫哥洛夫解释}

\begin{editorial}
  使用 BHK 解释证明直觉主义命题的有效性令人困惑；必须把它们解释得更好。
\end{editorial}

对于直觉主义联结词，存在一种非形式的构造性解释，通常称为布劳威尔–海廷–科莫哥洛夫（Brouwer--Heyting--Kolmogorov）解释，即 BHK 解释。它运用「构造」这一概念，你可以把它想成是一种构造性证明。（在 BHK 解释中我们不使用「证明」一词，以免与形式!!{derivation}系统中!!a{derivation}的概念相混淆。）基于这一直观概念，BHK 解释说明了直觉主义联结词的含义。

\begin{enumerate}
\item 我们假定，我们知道一个原子命题的构造由什么构成。
...
\begin{ex}
以 $\lnot \lfalse$ 为例。它的一个构造是一个函数，它把 $\lfalse$ 的任意一个构造作为输入，输出的则是~$\lfalse$ 的一个构造。显然，恒等函数~$\Id{}$ 就是这样的构造：对~$\lfalse$ 的一个构造~$M$ 输入 $\Id{}(M) = M$，得到一个~$\lfalse$ 的构造。
\end{ex}

一般说来，$\lnot !A$ 意为「构造 $!A$ 是不可能的」。
