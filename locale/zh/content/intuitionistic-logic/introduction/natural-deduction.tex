...
每种情形下，我们都可以把某条规则设想为允许我们断言：若前提有构造，则
结论也有构造。

由于自然演绎中的!!{derivation}都带有未消去的假设，我们应当把这样的!!a{derivation}，例如从!!{undischarged}假设~$\Gamma$ 推出的~$!A$，看作
把 $\Gamma$ 中所有 $!B \in \Gamma$ 的构造变为~$!A$ 的一个构造的函数。若
存在从无!!{undischarged}假设推出的~$!A$ 的!!a{derivation}，则在 BHK
...
若我们有从!!{undischarged}假设~$!A$ 推出~$!B$ 的!!a{derivation}，则
...
若我们有~$!A_i$ 的一个构造~$N_i$，就可以把它变为~$!A_1 \lor !A_2$ 的一个
...
若我们有从!!{undischarged}假设~$!B_1$、\dots、$!B_n$
推出~$\lfalse$ 的!!a{derivation}，则就有一个函数~$f(M_1, \dots, M_n)$ 把
$!B_1$、\dots、$!B_n$ 的构造变为~$\lfalse$ 的一个构造。由于 $\lfalse$ 没有构造，
$!B_1$、\dots、$!B_n$ 全部同时有构造也不可能。因此，$f$ 也具有如下性质：
\emph{若} $M_1$、\dots、$M_n$ 分别是 $!B_1$、\dots、$!B_n$ 的构造，
...
\begin{prop}
  若 $\Gamma \Proves !A$ 在直觉主义逻辑的自然演绎中成立，则 $\Gamma
  \Proves !A$ 在经典逻辑中也成立。特别是，若 $!A$ 是直觉主义定理，则
  $!A$ 也是经典定理。
\end{prop}
