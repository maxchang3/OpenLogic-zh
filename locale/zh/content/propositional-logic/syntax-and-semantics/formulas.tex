...
命题逻辑的 !!^{formula} 由
\emph{!!{propositional
    variable}}\iftag{prvFalse}{\iftag{prvTrue}{，}{和}命题常元~$\lfalse$}{}\iftag{prvTrue}{\iftag{prvFalse}{
    和}{}命题常元~$\ltrue$}{} 借助\emph{逻辑联结词}构成。
...
\begin{intro}
你可能熟悉与我们上面所用不同的术语和符号。逻辑教科书（和老师们）通常使用 $\sim$、$\neg$ 以及~$!$ 来表示「negation」，用 $\wedge$、$\cdot$ 和~$\&$ 来表示「conjunction」。「conditional」或「implication」常用的符号是 $\rightarrow$、$\Rightarrow$ 和~$\supset$。
\iftag{prvIff,defIff}{「biconditional」「bi-implication」或「(material) equivalence」的符号是 $\leftrightarrow$、
  $\Leftrightarrow$ 和~$\equiv$。}{}
\iftag{prvFalse,defFalse}{$\lfalse$ 符号有不同的叫法：
  「falsity」「falsum」「absurdity」或「bottom」。}{}
\iftag{prvTrue,defTrue}{$\ltrue$ 符号有不同的叫法：
  「truth」「verum」或「top」。}{}
\end{intro}
...
\item 每个命题变元~$\Obj p_i$ 都是一个原子!!{formula}。

\tagitem{prvNot}{若 $!A$ 是一个!!{formula}，则 $\lnot !A$ 是一个!!{formula}。}{}
...
\tagitem{limitClause}{除此之外没有别的!!{formula}。}{}
\end{enumerate}
\end{defn}
...
$\ident$ 符号两侧可由通过
连接而得到的字符串组成，例如 $!A \ident (!B \lor !C)$ 的意思是：符号串~$!A$ 与把左括号、字符串 $!B$、$\lor$ 符号、字符串~$!C$ 以及右括号依次连接而得到的那个字符串是同一个字符串。若如此，
那么我们就会知道 $!A$ 的第一个符号是左括号，$!A$ 含有 $!B$ 作为其一个子字符串（从第二个符号开始），该子字符串之后是~$\lor$，等等。
