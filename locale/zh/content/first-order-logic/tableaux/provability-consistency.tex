...
我们现在要确立!!{derivability}关系的若干性质。它们各自本身就很有意思，但每一个都将在完全性定理的证明中发挥作用。

\begin{prop}\ollabel{prop:provability-contr}
  若 $\Gamma \Proves !A$ 且 $\Gamma \cup \{!A\}$ 是不一致的，则 $\Gamma$ 是不一致的。
\end{prop}
...
另一方面，若后者的闭!!{tableau}存在，我们可以去掉每一个由\TRule{\True}{\lnot}作用于第一个假设~$\sFmla{\True}{\lnot !A}$而得到的公式以及该假设本身，并添加假设 $\sFmla{\False}{!A}$，从而把它转变为前者的闭!!{tableau}。因为若某条支之前由于包含 \TRule{\True}{\lnot} 作用于 $\sFmla{\True}{\lnot !A}$ 的结论（即 $\sFmla{\False}{!A}$）而是闭的，则新!!{tableau}中对应的支也是闭的。若旧表列中的某条支由于同时包含假设 $\sFmla{\True}{\lnot !A}$ 与 $\sFmla{\False}{\lnot
  !A}$ 而是闭的，我们可以通过把\TRule{\False}{\lnot} 应用于 $\sFmla{\False}{\lnot !A}$ 以得到
$\sFmla{\True}{!A}$，从而把它转变为闭的支。由于我们添加了
...
\begin{prop}\ollabel{prop:explicit-inc}
  若 $\Gamma \Proves !A$ 且 $\lnot !A \in \Gamma$，则 $\Gamma$ 是
  不一致的。
\end{prop}
...
\begin{proof}
  ...
  \TRule{\True}{\lnot} 作用于 \sFmla{\False}{!A} 的结论。在!!{tableau}中通过诉诸旧!!{tableau}中的第~$1$ 行而得到辩护的任何!!{sentence}，现在都通过诉诸第~$n+1$ 行而得到辩护。因此，若旧!!{tableau}是闭的，新表列也是闭的。它表明 $\Gamma$ 是不一致的，因为所有假设都在~$\Gamma$ 中。
\end{proof}

\begin{prop}\ollabel{prop:provability-exhaustive}
  若 $\Gamma \cup \{!A\}$ 与 $\Gamma \cup \{\lnot !A\}$ 两者都是
  不一致的，则 $\Gamma$ 是不一致的。
\end{prop}

\begin{proof}
  若存在 $!B_1$, \dots, $!B_n \in \Gamma$ 以及 $!C_1$, \dots,
  $!C_m \in \Gamma$，使得
  \begin{align*}
    \{\sFmla{\True}{!A}, &
    ...
    \sFmla{\True}{!C_1}, \dots, \sFmla{\True}{!C_m}\}
  \end{align*}
  两者都具有闭!!{tableau}，则我们可以通过以下方式构造单一的、组合的!!{tableau}，它表明 $\Gamma$ 是不一致的：以
  $\sFmla{\True}{!B_1}$, \dots, $\sFmla{\True}{!B_n}$
  ...
  因为它包含\Cut{}规则的右侧结论
  $\sFmla{\False}{!A}$。若第二个!!{tableau}中的某条支由于任何其他原因闭合，则组合!!{tableau}中对应的支也闭合，因为除 $\sFmla{\True}{\lnot !A}$ 之外在旧第二个!!{tableau}的该支上出现的任何!!{signed formula}也出现在组合!!{tableau}的对应支上。
  \end{proof}
