...
\begin{ex}
让我们为语句 $(!A \land !B) \lif !A$ 找一个闭的!!{tableau}。
...
\end{oltableau}
为了记录哪些!!{signed formula}我们已经应用了它们相应的规则，我们在语句旁边写一个对勾。但是，\emph{只有}当规则已被应用到所有开放的支时，才写对勾。一旦!!a{signed formula}的相应规则已在每个开放的支中被应用，我们就不必再回到它并再次应用规则。在这种情况下，只有一个支，所以规则只需被应用一次。（注意，对勾只是为了构造!!{tableau}的方便，并不正式属于表列的句法的一部分。）
...
我们可以针对任意多个!!{signed formula}作为假设给出!!{tableau}。通常也有必要应用不止一条允许分支的规则；一般来说，!!a{tableau}可以有任意多个支。例如，考虑~$\{\sFmla{\True}{!A \lor (!B \land !C)}, \sFmla{\False}{(!A \lor !B) \land (!A
  \lor !C)}\}$ 的一个!!{tableau}。我们先把 $\TRule{\True}{\lor}$ 应用于第一个假设：
\begin{oltableau}
