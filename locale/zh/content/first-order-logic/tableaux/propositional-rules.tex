...
\Cut{} 规则不是应用于先前的某个!!{signed formula}，而是让!!a{tableau}中的每一条支都一分为二，一条支包含 $\sFmla{\True}{!A}$，另一条支包含~$\sFmla{\False}{!A}$。它并非必不可少——任何具有闭的!!{tableau}的!!{signed formula}集都有一个不使用 \Cut{} 的表列——但它能以方便的方式让我们把!!{tableau}组合起来。
