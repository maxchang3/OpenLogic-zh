...
对于「仅当」方向，我们以!!{derivation}的长度进行归纳，其中!!{derivation}是 $!B$ 从 $\Gamma \cup \{!A\}$ 出发的推演。
...
\item 如果 $\Gamma \cup \{ \lnot !A\}
  ...
  !A$（换质位（Contraposition））； \ollabel{derivfacts:b}
\item  $\{ !A, \lnot!A\} \Proves
    !B$（爆炸原理（Ex Falso Quodlibet））； \ollabel{derivfacts:c}
\item  $\{ \lnot\lnot!A\} \Proves
  !A$（双重否定消去（Double Negation Elimination））；\ollabel{derivfacts:d}
