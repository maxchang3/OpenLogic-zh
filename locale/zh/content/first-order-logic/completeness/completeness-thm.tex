...
设 $\Gamma$ 是一组!!{sentence}。若 $\Gamma$ 是一致的，它就是可满足的。
\end{thm}
...
对所有 $\Gamma$ 和!!{sentence}~$!A$：若 $\Gamma \Entails !A$，则
$\Gamma \Proves !A$。
\end{cor}

\begin{proof}
注意 \olref{cor:completeness} 和 \olref{thm:completeness} 中的
$\Gamma$ 是全称量化的。为了不把我们自己弄糊涂，让我们换用不同的变元来重新表述 \olref{thm:completeness}：对任何一组!!{sentence}~$\Delta$，若
$\Delta$ 是一致的，它就是可满足的。由逆否，若
$\Delta$ 是不可满足的，则 $\Delta$ 就是不一致的。我们将用这个来证明该推论。
