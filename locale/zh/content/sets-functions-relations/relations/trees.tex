...
树中的祖先关系是严格偏序。这激发了集合论的定义。为陈述它，我们需要两个概念。在由~$\le$ 偏序的集合~$A$ 中，\emph{最小元}是满足如下条件的!!a{element} $x \in A$：对所有 $y \in A$ 都有 $x \le y$。一个集合由~$\le$ \emph{良序}，如果它的每一个非空子集都有一个最小元。
...
$x \in A$ 的后继也称为它的\emph{子节点}。如果 $y$ 是~$x$ 的一个后继，那么我们称 $x$ 是~$y$ 的\emph{前驱点}或\emph{父节点}。

\begin{prop}
如果 $\tuple{A,\le}$ 是一棵树，那么除根之外的每个 $x \in A$ 至多有一个前驱点。
\end{prop}
