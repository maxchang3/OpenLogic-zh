...
%
无穷树已经出现在命题逻辑和一阶逻辑完全性定理的证明中，并贯穿于整个数理逻辑的使用之中。
...
最底层节点~$r$ 是根。除~$r$ 之外的每个节点在紧邻其下方都恰好有一个父节点。我们可以把节点~$x$ 与节点~$y$ 之间的这样一种关系（即如果从~$x$ 沿边向上能到达~$y$）理解为~$x$ 是~$y$ 的一个\emph{祖先}。

树中的祖先关系是严格偏序。这激发了集合论的定义。为陈述它，我们需要两个概念。在由~$\le$ 偏序的集合~$A$ 中，\emph{最小元}是使对所有 $y \in A$ 都有~$x \le y$ 成立的!!a{element} $x \in A$。一个集合由~$\le$ \emph{良序}，如果它的每一个非空子集都有一个最小元。

\begin{defn}[树]
一棵\emph{树}是一个二元组 $T = \tuple{A, \le}$，其中 $A$ 是集合，$\le$ 是~$A$ 上的偏序，且具有唯一的最小元 $r \in A$（称为\emph{根}），使得对所有 $x \in A$，集合 $\Setabs{y}{y \le x}$ 都由~$\le$ 良序。
\end{defn}

\begin{defn}[后继]
设 $T = \tuple{A, \le}$ 是一棵树。
如果 $x,y \in A$，$x < y$，且不存在 $z \in A$ 使 $x < z < y$，那么我们说 $y$ 是~$x$ 的一个\emph{后继}。
\end{defn}

$x \in A$ 的后继也称为它的\emph{子节点}。如果 $y$ 是~$x$ 的一个后继，那么我们称 $x$ 是~$y$ 的\emph{前驱}或\emph{父节点}。

\begin{prop}
如果 $\tuple{A,\le}$ 是一棵树，那么除根之外的每个 $x \in A$ 至多有一个前驱。
\end{prop}

\begin{proof}
  设 $y_1 < x$ 且 $y_2 < x$ 且 $y_1 \neq y_2$。则 $\{y_1,
  y_2\} \subseteq \Setabs{z}{z<x}$。由于 $\Setabs{z}{z<x}$ 由~$\le$ 良序，其子集 $\{y_1, y_2\}$ 有一个最小元，显然它必须是 $y_1$ 或~$y_2$。所以要么 $y_1 \le y_2$，要么 $y_2 \le y_1$。我们假设了 $y_1 \neq y_2$，因此实际上要么 $y_1 < y_2$，要么 $y_2 < y_1$。由于我们假设了 $y_1 < x$ 和 $y_2 < x$，我们进而有要么 $y_1 < y_2
  < x$，要么 $y_2 < y_1 < x$。所以 $y_1$ 和 $y_2$ 不可能都是~$x$ 的前驱。
\end{proof}

\begin{defn}
如果 $A$ 是无穷集合，则称树 $T = \tuple{A, \le}$ 是\emph{无穷的}，否则称它是\emph{有穷的}。如果 $T$ 使得每个 $x \in A$ 只有有穷多个后继，那么我们说 $T$ 是\emph{有穷分支的}。
\end{defn}
...
如果 $T = \tuple{A,\le}$ 是一棵有穷分支的无穷树，那么 $T$ 有一个无穷支。
\end{prop}
