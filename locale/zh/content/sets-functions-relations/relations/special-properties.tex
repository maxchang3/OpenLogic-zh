...
\begin{defn}[传递性]
关系 $R \subseteq A^2$ 是\emph{传递的}，当且仅当每当 $Rxy$ 与 $Ryz$ 时，也有 $Rxz$。
\end{defn}

\begin{defn}[对称性]
关系~$R \subseteq A^2$ 是\emph{对称的}，当且仅当每当 $Rxy$ 时，也有 $Ryx$。
\end{defn}

\begin{defn}[反对称性]
关系~$R \subseteq A^2$ 是\emph{反对称的}，当且仅当每当 $Rxy$ 与 $Ryx$ 同时成立时，$x=y$（或者换句话说：若 $x\neq y$，则要么 $\lnot Rxy$ 要么 $\lnot Ryx$）。
\end{defn}
...
\begin{defn}[禁自反性]
若对所有 $x \in A$ 都不成立 $Rxx$，则关系 $R \subseteq A^2$ 被称为\emph{禁自反的}。
\end{defn}

\begin{defn}[禁对称性]
若不存在 $x,y\in A$ 使得 $Rxy$ 与 $Ryx$ 同时成立，则关系 $R \subseteq A^2$ 被称为\emph{禁对称的}。
\end{defn}

注意，若 $A \neq \emptyset$，则~$A$ 上没有任何禁自反关系是自反的，并且~$A$ 上的每个禁对称关系也都是反对称的。但是，存在既不自反也不禁自反的 $R \subseteq A^2$，也存在不禁对称的反对称关系。
