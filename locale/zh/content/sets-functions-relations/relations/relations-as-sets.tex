...
有时把 $\tuple{x, y} \in R$ 写作 $Rxy$（或 $xRy$）。
\end{defn}
...
\[
  ...
  \]
是~$\Nat$ 上的\emph{恒等关系}。（由于恒等关系很常用，定义 $\Id{A}=\Setabs{\tuple{ x,x }}{x \in
...
称 $K=L\cup I$ 为\emph{小于或等于}关系：当且仅当 $n \le m$
时 $Knm$。类似地，$H=G \cup
...
没有任何元素对处于其中），而 $A^{2}$~本身也是~$A$ 上的一个关系
