...
\olsection{Russell 悖论}

外延性限定符号 $\Setabs{x}{\phi(x)}$，它表示那些满足~$\phi(x)$ 的 $x$ 所构成的\emph{那个}集合。然而，外延性所\emph{真正}限定的只是下述想法。\emph{如果}存在一个集合，其成员恰好是那些 $\phi$，\emph{那么}这样的集合只有一个。换言之：给定某个~$\phi$ 后，集合 $\Setabs{x}{\phi(x)}$ 是唯一的，\emph{如果它存在的话}。

但这个条件句很重要！{}关键在于，并非每个性质都适用于\emph{概括}。也就是说，有些性质\emph{不}定义集合。如果它们全都定义集合，那么我们就会陷入彻头彻尾的矛盾之中。最著名的例子便是 Russell 悖论。
...
当我们考虑「不以其自身为!!a{element}」这一性质，即\emph{非自属的}这一性质时，Russell 悖论就出现了。如果设想存在一个集合，其中的元素都是不以其自身为!!a{element}的集合，那么会怎样呢？那
\[
R = \Setabs{x}{x \notin x}
\]
