...
$\tuple{\tuple{\tuple{x,y},z},u}$，等等。一般地，称\emph{有序 $n$ 元组}$\tuple{x_1, \dots, x_n}$。
...
\begin{ex}
若 $A$ 是一个集合，则$A$ 上的一个\emph{词}是$A$ 的!!{element}的任意序列。一个序列可以看作为$A$ 的!!{element}的一个 $n$ 元组。例如，若 $A = \{a, b, c\}$，则序列 「$bac$」 可以看作三元组~$\tuple{b, a, c}$。词，即符号序列，在计算机科学中至关重要。按约定，$A$ 的!!{element}计为长度为~$1$ 的序列，$\emptyset$ 计为长度为~$0$ 的序列。则$A$ 上\emph{所有}词的集合为
\[
